...
{
    public partial class controlPublicidadConsulta : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                dt.Rows.Add("@vista", "VarChar", "vwControlPublicidad");
        ...
        protected void gvControlPublicidad_PageIndexChanging(Object sender, GridViewPageEventArgs e)
        {
            if (e.NewPageIndex != -1)
                gvControlPublicidad.PageIndex = e.NewPageIndex;
        }
        
        protected void gvControlPublicidad_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (Convert.ToInt32(Session["idPerfil"]) != 2 && Convert.ToInt32(Session["idPerfil"]) != 6)
            {
                ...
                {
                    int idCP = Convert.ToInt32(e.CommandArgument), tipo = 1;
                    Response.Redirect("~/WebForms/controlPublicidad/controlPublicidadActualizar.aspx?idCP=" + idCP + "&tipo=" + tipo, false);
                }
            }
            else
                mensajeError(@"ERROR: No se puede realizar la acción solicitada, no cuenta con el permiso correspondiente.");
        ...
        protected void gvControlPublicidad_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if (e.Row.Cells[20].Text.Length > 0)
                {
                    string caracter = e.Row.Cells[20].Text.Substring(0, 1);
        ...
        private void LlenarGrid()
        {
            ...
            dt = action.EjecutarSP("spConsultaVistaControlpublicidad", dtSP);

            /*gvHotel.DataTextField = "atsNombreHotel";
            gvHotel.DataValueField = "skNIdHotel";*/
            gvControlPublicidad.DataSource = dt;
            gvControlPublicidad.DataBind();

            if (gvControlPublicidad.Rows.Count > 0)
            {
                for (int i = 1; i <= Convert.ToInt16(Session["numCols"]); i++)
                {
                    ...
                    ddl = (DropDownList)gvControlPublicidad.HeaderRow.FindControl("ddl" + Convert.ToString(i));
        ...
        private void LlenarListado(DropDownList ddl, DataTable dt, int index)
        {
            ...
            dtList = action.EjecutarSP("spConsultaVistaControlPublicidad", dt);
