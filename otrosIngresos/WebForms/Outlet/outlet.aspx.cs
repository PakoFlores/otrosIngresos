    ...
    public partial class outlet : System.Web.UI.Page
    {
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlEquipo.SelectedItem.Value == "5")
                {
                    ...
                    dtMedia.Rows.Add("@nombreCliente", "VarChar", txtCliente.Text);
        ...
        protected void ddlTipoAcuerdo_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                dt = action.Consultar(
                    ...
                    + "WHERE atBEstatus = 1 AND skNIdTipoAcuerdo IN(2)");
        ...
        protected void btnOrden_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (fuSubirOrden.HasFile == true)
                {
                        ...
                        extensionArchivo == ".tif" || extensionArchivo == ".jfif" || extensionArchivo == ".tiff")
                    {
                        if (fuSubirOrden.PostedFile.ContentLength < 10000000)
                        {
                            ...
                            //fuSubirAcuerdo.SaveAs(savePath);
                ...
                {
                    lbSubirOrden.Text = "Tipo de archivo no válido para cargar.";
                }
            }
        ...
        private void preparaPDF(string accion)
        {
            try
            {
                ...
                dtSP.Rows.Add("@usuario", "Int", Convert.ToInt32(Session["idUsuario"]));
                dtSP.Rows.Add("@cliente", "VarChar", txtCliente.Text);
                ...
                {
                    dtSP.Rows[0]["Cliente"] = txtCliente.Text;
                ...
                string filenameM = generarPDF(dtSP, dt, dtgv, accion, 1);
                string outputFileM = this.Server.MapPath("~/Documents/Files/ControlPublicidad/" + filenameM);
                string filenameD = generarPDF(dtSP, dt, dtgv, accion, 2);
                string outputFileD = this.Server.MapPath("~/Documents/Files/ControlPublicidad/" + filenameD);
        ...
        public string generarPDF(DataTable dtSP, DataTable dt, DataTable dtgv, string accion, int i)
        {
            ...
            if (i == 1)
            {
                nombreArchivo = rowSP[1].ToString() + "_" + System.DateTime.Now.ToString("yyyyMMdd") + "_Montos.pdf";
                outputFile = this.Server.MapPath("~/Documents/Files/ControlPublicidad/" + nombreArchivo);
                lbRutaOrden.Text = nombreArchivo;
            }
            else
            {
                nombreArchivo = rowSP[1].ToString() + "_" + System.DateTime.Now.ToString("yyyyMMdd") + "_Detalle.pdf";
                outputFile = this.Server.MapPath("~/Documents/Files/ControlPublicidad/" + nombreArchivo);
